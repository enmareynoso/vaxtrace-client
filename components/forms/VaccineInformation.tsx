...
export default function VaccineInformation({
  ...
  const fetchRecommendedVaccines = async (
    ...
    const recommendedVaccines = allVaccines.filter((vaccine) => {
      ...
      return (
        ...
        appliedDoses < vaccine.max_doses
      ); // Excluir si ya alcanzó la dosis máxima
    });
  ...
  const getFilteredVaccines = (index: number) => {
    ...
    return availableVaccines.filter(
      (vaccine) =>
        !selectedIds.includes(vaccine.vaccine_id) ||
        vaccineSelections[index] === vaccine.vaccine_id.toString()
    );
  };
